...
import { SiAudiomack, SiTurborepo } from "react-icons/si";
...
function App() {
  ...
  const handleEmbeding = async () => {
    setShowModal(false);
    setEmbedding(true);
    if (channelID == "") {
      alert("Input channel ID correctly");
      return;
    }
    await axios
      .post("http://localhost:8000/make_embedding", {
        channel_id: channelID,
      })
      .then((res) => {
        alert("Embedding was completed successfully.");
        setEmbedding(false);
      })
      .catch((err) => {
        console.log(err);
  ...
  const chat = async (e, message) => {
    ...
    await axios
      .post("http://localhost:8000/get_answer", { message })
      .then((result) => {
        if (voiceId) {
          fetch(
            `${
  ...
  return (
    ...
    >
      ...
      >
        <h1>Voice ChatBot</h1>
        <button className="modal_button" onClick={handleButtonClick}>
          Set Channel ID
        </button>
      </div>
      ...
      {embedding == true ? (
        <img src={LoadingImage} alt="loading" width={40} className="loading" />
      ) : (
        <span />
      )}
          ...
          <div className="modal-content">
            ...
            <p>
              <div>
                ...
                />
                <button className="embedding_btn" onClick={handleEmbeding}>
                  Embedding
                </button>
              </div>
            </p>
